 ...
 * @author Oscar
 */
public class ControladorConfirmarPlan extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ControladorConfirmarPlan</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ControladorConfirmarPlan at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("Confirmar")) {
            ...
            double total = Double.parseDouble(adultos) * Double.parseDouble(precioA) + Double.parseDouble(ninos) * Double.parseDouble(precioN);
            
            PlanesTuristicosDAO dao = new PlanesTuristicosDAO();
            if(dao.agregarReserva(idUser,idPlan,adultos,ninos,total))
            {
                acceso = inicio;
            }
            else
